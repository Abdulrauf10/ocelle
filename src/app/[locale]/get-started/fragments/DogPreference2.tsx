...
export default function DogPreference2Fragment() {
  ...
  const { name, foodAllergies, currentEating, amountOfTreats, pickiness, recipe1, recipe2 } =
    getDog();
  const form = useForm<DogPreference2Form>({
    defaultValues: {
              ...
              <div className="mx-auto -mt-4 flex max-w-[360px] flex-wrap justify-center">
                <div className="mt-4 px-3">
                  <InteractiveBlock
                    ...
                    rules={{
                      validate: {
                        required: (value, formValues: DogPreference2Form) =>
                          formValues.allergies.some((value) => !!value),
                        conflict: (value, formValues: DogPreference2Form) =>
                  ...
                  return (
                    <div className="mt-4 px-3" key={idx}>
                      <InteractiveBlock
                        ...
                        rules={{
                          validate: {
                            required: (value, { allergies }: DogPreference2Form) =>
                              allergies.some((value) => !!value),
                            conflict: (value, { allergies }: DogPreference2Form) =>
                              !value || !allergies[0],
                            allAllergies: (value, { allergies }: DogPreference2Form) => {
                              const foodAllergies = arrayToAllergies(allergies);
